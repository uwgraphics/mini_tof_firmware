...
extern DigitalOut debug_trigger;
		...
		uint8_t					expected_value)
{
	...
	uint8_t timeout = 0;
    //printf("Polling\n");
    // printf("\nPoll Timeout = %u ---- Expected Value = %x\n", timeout, expected_value);
    // printf("temp_buffer[pos] & mask = %x\n\n", p_dev->temp_buffer[pos] & mask);
	do {
        //printf("\nPoll Timeout = %u ---- Expected Value = %x\n", timeout, expected_value);
		...
		VL53LMZ_Configuration		*p_dev)
{
    ...
    printf("Status at donwload firmware = %u\r\n", status);
    ...
    printf("Status before Get offset NVMdata = %u\r\n", status);
    ...
    printf("Before going into send_offset_data = %u\r\n", status);

	status |= _vl53lmz_send_offset_data(p_dev, VL53LMZ_RESOLUTION_4X4);

    printf("Status after Get offset NVMdata = %u\r\n", status);
    ...
    //printf("Status after Xtalk NVMdata = %u\r\n", status);
    ...
    //printf("Poll_for_answer @ UI_CMD_STATUS = %u\r\n", status);
